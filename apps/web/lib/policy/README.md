...
- `resolveRequiredInfoModeForSave(fieldId, requestedMode)`: the save-time
  gate actually wired into
  [`apps/web/lib/repository/postgres.ts`](../repository/postgres.ts)'s
...
and is exported (see below), but nothing in this codebase currently trusts it
...
`RequiredInfoAnswer.mode = auto` be saved and honored as-is; require issue
#4's apply-agent to call `isCriminalHistoryAutoModeAllowed(job.jurisdiction)`
...
this field is used for anything — today that's one apply-agent (issue #4,
being built in parallel by another agent right now), but the surface only
grows (exports, admin tooling, a future "preview my answers" feature, a bug
that reorders a check after a fill instead of before). A missed call
anywhere in that surface silently reintroduces exactly the harm this issue
exists to prevent, and — because `JURISDICTION_POLICY` starts empty — B
would also be currently indistinguishable from "fully enforced" in testing,
since every check trivially returns `false` today; the gap only shows up
once someone populates the map and simultaneously forgets a call site. A
fails safe at the one place this data is written, independent of how many
future consumers exist or whether they remember to ask.
...
**What this means for issue #4 (apply-agent):** `resolveRequiredInfoModeForSave`
means you should never actually see `mode = "auto"` for `criminal_history`
coming back from the repository today. Call
...
sign-off exists. Never fall back to auto-filling this field from a stale or
cached answer if the check returns `false` — re-prompt the user instead, the
same way `mode = "manual"` does for every other field.
